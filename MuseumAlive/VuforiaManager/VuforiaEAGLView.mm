...
- (void)setNeedsChangeSceneWithUserInfo: (NSDictionary*)userInfo {
	...
	//NSLog("overlay child count: %d",self.overlayScene.children.count)
	
	_renderer.overlaySKScene = self.overlayScene;
...
- (SCNHitTestResult*)touchedNodeWithLocationInView:(CGPoint)location {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    SCNHitTestResult* result = [self touchedNodeWithLocationInView:pos];
    if(result.node){
        _currentTouchNode = result.node;
        [self.delegate vuforiaEAGLView:self didTouchDown:result];
    }
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    CGPoint pos = [touches.anyObject locationInView:self];
	SCNHitTestResult* result = [self touchedNodeWithLocationInView:pos];
    if(_currentTouchNode == result.node){
        [self.delegate vuforiaEAGLView:self didTouchUp:result];
    }else {
