...
- (void)configureVideoBackgroundWithViewWidth:(float)viewWidth andHeight:(float)viewHeight
{
	...
	//Vuforia::CameraDevice::getInstance().selectVideoMode(Vuforia::CameraDevice::getInstance().MODE_OPTIMIZE_SPEED);
	int x = Vuforia::Renderer::getInstance().setTargetFps(30);
	NSLog(@"result from fps set: %d",x);
}
