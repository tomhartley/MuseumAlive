...
static const GLsizei SHADERS_BUFFER_NUM = 6;
static const int GEOMETRY_ARRAY = 0;
static const int NORMALS_ARRAY = 1;
static const int TEXTURE_ARRAY = 2;
static const int OBJ_MTL_EXTRA_ARRAY = 3;
static const int OBJ_AMBIENT_ARRAY = 4;
static const int OBJ_DIFFUSE_ARRAY = 5;
...
@property (nonatomic) GLint objMtlTexCoordHandle;
...
- (bool) loadModel: (NSString *) filename {
    ...
    for(int n = 0, i=0; n < self.nbMaterials ; n++) {
...
- (bool) initShaders {
    ...
    self.objMtlProgramID = [SampleApplicationShaderUtils createProgramWithVertexShaderFileName:@"DiffuseLight.vertsh"
    ...
    self.objMtlTexCoordHandle = glGetAttribLocation(self.objMtlProgramID, "a_texCoord");
...
- (bool) renderWithModelView:(float[]) modelViewMatrix modelViewProjMatrix:(float[]) modelViewProjMatrix
{
    ...
    if(self.isLoaded) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, self.shaderBuffers[TEXTURE_ARRAY]);
        glVertexAttribPointer(self.objMtlTexCoordHandle, 2, GL_FLOAT, false, 0,
                              0);
        ...
        glEnableVertexAttribArray(self.objMtlTexCoordHandle);
        ...
        glDisableVertexAttribArray(self.objMtlTexCoordHandle);
