...
- (void) initAR:(int) VuforiaInitFlags orientation:(UIInterfaceOrientation) ARViewOrientation {
    ...
    self.cameraMode = Vuforia::CameraDevice::MODE_OPTIMIZE_SPEED;
...
- (bool) resumeAR:(NSError **)error {
    ...
    if ((self.cameraIsStarted) && (! self.cameraIsActive)) {
        ...
        if (! cameraDevice.init(mCamera)) {
            [self NSErrorWithCode:E_INITIALIZING_CAMERA error:error];
            return NO;
        }
        
        // select the video mode
        if(! cameraDevice.selectVideoMode(self.cameraMode)) {
...
- (bool)startCamera:(Vuforia::CameraDevice::CAMERA_DIRECTION)camera viewWidth:(float)viewWidth andHeight:(float)viewHeight error:(NSError **)error
{
    ...
    // configure Vuforia video background
    [self.delegate configureVideoBackgroundWithViewWidth:viewWidth andHeight:viewHeight andCameraMode:self.cameraMode];
...
- (void) changeOrientation:(UIInterfaceOrientation) ARViewOrientation {
    ...
    [self.delegate configureVideoBackgroundWithViewWidth:arViewBoundsSize.width andHeight:arViewBoundsSize.height andCameraMode:self.cameraMode];
