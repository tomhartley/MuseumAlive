...
- (void)renderFrameVuforiaInternal
{
    ...
    for (int viewIdx = 0; viewIdx < viewList.getNumViews(); viewIdx++) {
        ...
        Vuforia::Matrix34F projMatrix = self.currentRenderingPrimitives->getProjectionMatrix(vw,
                                                                                Vuforia::COORDINATE_SYSTEM_WORLD, state.getCameraCalibration());
        ...
        if (self.currentView != Vuforia::VIEW_POSTPROCESS) {
            ...
            Vuforia::Tool::convert2GLMatrix(self.currentRenderingPrimitives->getVideoBackgroundProjectionMatrix(vw,
                                                                                    Vuforia::COORDINATE_SYSTEM_CAMERA));
            
            [self.control renderFrameWithState:state projectMatrix:projectionMatrix vbProjectMatrix:vbProjectionMatrix];
        }
